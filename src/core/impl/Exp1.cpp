...
AdvX::Exp1::actual_advection(sycl::queue &Q,
    ...
    if (wg_size_y * nx > 6144) {
        std::cout << "wg_size_y = " << wg_size_y << ", nx = " << nx << std::endl;
        throw std::invalid_argument(
            "wg_size_y*nx must be < to 6144 (shared memory limit)");
    }
    // if (wg_size_y * nx > 6144) {
    //     std::cout << "wg_size_y * nx > 6144" << std::endl;
    // }
    // else {
    //     std::cout << "wg_size_y * nx <= 6144" << std::endl;
    // }
    ...
    //     *ptr_buff_rest_nx = sycl::buffer<double, 2>(sycl::range{ny, nx_rest_to_malloc}, sycl::no_init);
        ...
        cgh.parallel_for_work_group(nb_wg, wg_size, [=](sycl::group<3> g) {
            ...
            g.parallel_for_work_item(
                sycl::range{wg_size_y, nx, 1}, [&](sycl::h_item<3> it) {
                    ...
                    const int ivx = wg_size_y * g.get_group_id(0) + ny_offset + local_nb;
...
AdvX::Exp1::operator()(sycl::queue &Q,
    ...
    if (ny < MAX_NY) {
        ...
        auto n_batch =
            std::floor(ny / MAX_NY) + 1;   // should be in constructor

        // For BBlock in B
        for (int i_batch = 0; i_batch < n_batch - 1; ++i_batch) {
            // can we parallel_for this on multiple queues ? or other CUDA streams?

            size_t ny_offset = (i_batch * MAX_NY) - i_batch;

            std::cout << "here" << std::endl;
            actual_advection(Q, buff_fdistrib, params, MAX_NY-1, ny_offset, rest_malloc)
                .wait();
        }

        // for the last one we take the rest, we add n_batch-1 because we
        // processed MAX_SIZE-1 each batch
        auto const ny_size = (ny % MAX_NY) + (n_batch - 1);
        auto const ny_offset = (MAX_NY - 1) * (n_batch - 1);
        return actual_advection(Q, buff_fdistrib, params, ny_size, ny_offset, rest_malloc);
    }
}
