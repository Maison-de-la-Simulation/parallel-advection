...
void optimized_codepaths(double* ptr, int ib, int n1, int n2, int idx_ipos1, int is, double& value)
{
  __hipsycl_if_target_cuda(
        value = __ldg(ptr + (ib*n1*n2+idx_ipos1*n2+is));
  );
  __hipsycl_if_target_host(
        value = *(ptr + (ib*n1*n2+idx_ipos1*n2+is));
  );
}
#endif
            ...
            g.parallel_for_work_item(
                sycl::range{1, n1, 1}, [&](sycl::h_item<3> it) {
                    ...
                    for (int k = 0; k <= LAG_ORDER; k++) {
                        ...
                        double v = 0;
                        optimized_codepaths(ptr, ib, n1, n2, idx_ipos1, is, v);
                        // auto value = __ldg(ptr + (ib*n1*n2+idx_ipos1*n2+is));
                        // double value = __ldg(&fdist[i0][idx_ipos1][i2]);

                        // auto value = *(ptr + (ib*n1*n2+idx_ipos1*n2+is));

                        // slice_ftmp[i1] += coef[k] * fdist[ib][idx_ipos1][i2];
