...
// ==========================================
sycl::event
AdvX::StraddledMalloc::adv_opt3(sycl::queue &Q, double *fdist_dev,
    ...
    // TODO we don't want this, we want to allocate a 1D slice for each problem
    // in parallel, containing only the rest of n1 slice in 1D
    sycl::buffer<double, 3> buff_rest_nx(
        sycl::range<3>{n0, nx_rest_to_malloc, n2}, sycl::no_init);

    return Q.submit([&](sycl::handler &cgh) {

        sycl::local_accessor<double, 1> slice_ftmp(sycl::range<1>(MAX_NX_ALLOC),
                                                   cgh);
        sycl::accessor overslice_ftmp(buff_rest_nx, cgh, sycl::read_write,
                                      sycl::no_init);
        ...
        cgh.parallel_for_work_group(nb_wg, wg_size, [=](sycl::group<3> g) {
            g.parallel_for_work_item(
                sycl::range{1, n1, 1}, [&](sycl::h_item<3> it) {
                    ...
                    // if i1 > 6144; we use overslice_ftmp with index
                    // i1-MAX_NX_ALLOC else we use slice_ftmp
                    ...
                    if (i1 < MAX_NX_ALLOC) {
                        ...
                        for (int k = 0; k <= LAG_ORDER; k++) {
                            ...
                            overslice_ftmp[i0][i1 - MAX_NX_ALLOC][i2] +=
                                coef[k] * fdist(i0, id1_ipos, i2);
                        }
                    }
...
// ==========================================
sycl::event
AdvX::StraddledMalloc::operator()(sycl::queue &Q, double *fdist_dev,
                                  const ADVParams &params) {
    auto const n1 = params.n1;
    // auto const n0 = params.n0;
    // auto const n2 = params.n2;

    // On A100 it breaks if we allocate more than 48 KiB per block, which is
    // 6144 double On MI250x it breaks if we allocate more than 64KiB per wg,
    // which is 8192 double
    if (n1 <= MAX_NX_ALLOC) {
        ...
        //=================================
        // Option 3
        // we could launch two kernels in parallel, one using shared mem and one
        // using the rest in global mem
        auto rest_malloc = n1 - MAX_NX_ALLOC;
        return adv_opt3(Q, fdist_dev, params, rest_malloc);
    }
}
