...
sycl::event
advector::x::FixedMemoryFootprint::operator()(
    ...
    //                  first_row_number +
    //                  g.get_group_id(
    //                      0);   // where we should take the slice in memory
    ...
    //                          fdist[ivx][ix] +=
    //                              coef[k] *
    //                              buff_ftmp[idx_ipos1 + nx * row_number];
    //                      }
    ...
    return Q.submit([&](sycl::handler &cgh) { cgh.single_task([=]() {}); });
}