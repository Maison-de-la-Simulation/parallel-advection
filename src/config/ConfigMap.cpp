/**
 * \file ConfigMap.cpp
 * \brief Implement ConfigMap, essentially a INIReader with additional get
 * methods.
 *
...
ConfigMap::ConfigMap(std::string filename)
    : INIReader(filename) {}   // ConfigMap::ConfigMap

// =======================================================
// =======================================================
ConfigMap::ConfigMap(char *&buffer, int buffer_size)
    : INIReader(buffer, buffer_size) {}   // ConfigMap::ConfigMap
...
// =======================================================
double
ConfigMap::getFloat(std::string section, std::string name,
    ...
    return end > value ? static_cast<double>(valFloat)
...
// =======================================================
bool
ConfigMap::getBool(std::string section, std::string name,
                   bool default_value) const {
    bool val = default_value;
    std::string valstr = getString(section, name, "");

    if (!valstr.compare("1") or !valstr.compare("yes") or
        !valstr.compare("true") or !valstr.compare("on"))
        val = true;
    if (!valstr.compare("0") or !valstr.compare("no") or
        !valstr.compare("false") or !valstr.compare("off"))
        val = false;
